...
class Drawer:
    ...
    def redraw(self):
        ...
        self.image_editor.tk_img = ImageTk.PhotoImage(cropped.resize((400, 400), Image.LANCZOS))
        self.image_editor.canvas.create_image(0, 0, anchor=tk.NW, image=self.image_editor.tk_img)
        self.draw_graph()
    ...
    def draw_node(self, node, selected=False):
        ...
        if 0 <= x <= 400 and 0 <= y <= 400:
            ...
            self.image_editor.canvas.create_oval(x - 5, y - 5, x + 5, y + 5, fill=color, outline="")
    ...
    def draw_edge(self, edge):
        ...
        if (0 <= start_x <= 400 and 0 <= start_y <= 400) or (0 <= end_x <= 400 and 0 <= end_y <= 400):
            self.image_editor.canvas.create_line(start_x, start_y, end_x, end_y, fill="red", width=2)
            edge_key = self.image_editor.graph.edge_to_string(self.image_editor.graph.get_edge(edge[0], edge[1]))
            if edge_key in self.image_editor.graph.edge_labels:
                self.image_editor.canvas.create_text((start_x + end_x) / 2, (start_y + end_y) / 2,
